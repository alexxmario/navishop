...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get real featured products from MongoDB
    const featuredProducts = await Product.find({
      featured: true,
      status: 'active'
    })
    .select('name price originalPrice images slug averageRating totalReviews stock category brand')
    .limit(8)
    .lean();
    ...
    res.status(200).json(mockFeaturedProducts);
  }
}