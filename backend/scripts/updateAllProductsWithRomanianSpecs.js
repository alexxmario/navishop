...
// Create a template for products based on their specifications
function generateRomanianSpecsFromProduct(product) {
  ...
  if (product.technicalFeatures && product.technicalFeatures.length > 0) {
    ...
    if (product.technicalFeatures.some(f => f.toLowerCase().includes('android') && f.toLowerCase().includes('app'))) {
      romanianSpecs.features.suportAplicatiiAndroid = "Da, se poate instala orice aplicatie din magazinul Google Play";
      romanianSpecs.rawDetails["Suport Aplicatii Android"] = "Da, se poate instala orice aplicatie din magazinul Google Play";
    }

    if (product.technicalFeatures.some(f => f.toLowerCase().includes('voice') || f.toLowerCase().includes('siri'))) {
      romanianSpecs.features.limbiInterfata = "Peste 60, inclusiv limba romana";
      romanianSpecs.rawDetails["Limbi Interfata"] = "Peste 60, inclusiv limba romana";
    }
  }

  // Package content
  romanianSpecs.package.continutPachet = "2 Porturi USB, Antena Semnal GPS, Cablu Preluare Camera Marsarier, Conectica Dedicata, Microfon Extern, Rama Adaptoare, Tableta Android";
  romanianSpecs.rawDetails["Continut Pachet"] = romanianSpecs.package.continutPachet;

  romanianSpecs.package.formateMediaSuportate = "AVI, JPEG, MP3, MP4, MPEG1, MPEG2, MPEG4, PNG, WAV, WMA, WMV";
  romanianSpecs.rawDetails["Formate media suportate"] = romanianSpecs.package.formateMediaSuportate;
...
async function updateAllProductsWithRomanianSpecs() {
  try {
    console.log('Connecting to MongoDB...');
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/piloton');
    console.log('Connected to MongoDB');

    // Update all products without Romanian specifications
    console.log('Finding all products without Romanian specifications...');

    const products = await Product.find({
      $or: [
        { romanianSpecs: { $exists: false } },
        { "romanianSpecs.hardware": { $exists: false } }
      ]
    });
...
// Run the update
updateAllProductsWithRomanianSpecs();