...
const ProductReviewsTab = () => {
  ...
  const fetchReviews = useCallback(async () => {
    try {
      const response = await fetch(buildApiUrl(`reviews/product/${record.id}?page=1&limit=50`));
      if (response.ok) {
        const data = await response.json();
  ...
  const handleStatusChange = async (reviewId, newStatus) => {
    try {
      const response = await fetch(buildApiUrl(`reviews/admin/${reviewId}/status`), {
        method: 'PUT',
        headers: {
  ...
  const handleDeleteReview = async () => {
    try {
      const response = await fetch(buildApiUrl(`reviews/${deleteDialog.reviewId}`), {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
